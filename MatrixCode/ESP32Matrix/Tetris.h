...
class Tetris {
  ...
  uint8_t PlayfieldData[32 * ((MATRIX_WIDTH + 7) / 8) * _3BIT];
  uint8_t PlayfieldMask[32 * ((MATRIX_WIDTH + 7) / 8) * _1BIT];
  ...
  boolean loop() {
    if (_abs(millis() - LastLoop) >= LoopDelayMS)
      {
        LastLoop = millis();
        FastLED.clear();
    
        // Fill background with dim plasma
        #define TETRIS_PLASMA_X_FACTOR  24
        #define TETRIS_PLASMA_Y_FACTOR  24
        ...
        if(IS_PLASMA_BACKGROUND_NEEDED){ //////////Evgeny Added
            for (int16_t x=0; x<MATRIX_WIDTH; x++)
            {
              for (int16_t y=0; y<32; y++)
              {
                ...
                int16_t h = sin16(x * r * TETRIS_PLASMA_X_FACTOR + PlasmaTime) + cos16(y * (-r) * TETRIS_PLASMA_Y_FACTOR + PlasmaTime) + sin16(y * x * (cos16(-PlasmaTime) / 256) / 2);
        ...
        else
        {
          if (Sprites.IsSprite(&CompletedLines))  // We have highlighted complete lines, delay for visual effect
          {
            ...
            else
            {
              ...
              int16_t k;
              for (int16_t i=(32-1)*Dbpl,j=(32-1)*Mbpl; i>=0; i-=Dbpl,j-=Mbpl)
              {
          ...
          else
          {
            ...
            if (NextBlock)  // Start new block
            {
              if (CurrentBlock.GetXChange() >= 0) // We have a current block so add to playfield before creating new block
              {
                ...
                for (int16_t i=(32-1)*Mbpl, y=0; i>=0; i-=Mbpl,++y)
                {
              ...
              CurrentBlock.SetPositionFrameMotionOptions((MATRIX_WIDTH/2)-1, 32, 0, 0, 0, 0, -1, DropDelay, SPRITE_DETECT_COLLISION | SPRITE_DETECT_EDGE);
